...
%% @doc Start building an image from a base image reference.
%%
...
from({Registry, Repo, Tag} = Ref) when is_binary(Registry), 
                                        is_binary(Repo), 
                                        is_binary(Tag) ->
...
%% @doc Start building an image from a base image with authentication.
...
%% @doc Start building an image from scratch (no base image).
%%
...
%% @doc Add a layer to the image from a list of files.
%%
...
%% ]).
%% '''
-spec add_layer(image(), [{Path :: binary(), Content :: binary(), Mode :: integer()}]) -> image().
...
%% @doc Copy files to a destination directory in the image.
%%
...
-spec copy(image(), [{Name :: binary(), Content :: binary()}], Dest :: binary()) -> image().
copy(Image, Files, Dest) ->
    LayerFiles = lists:map(
        fun({Name, Content}) ->
...
%% @doc Set the entrypoint for the container.
%%
...
%% @doc Set the default command arguments.
%%
...
%% @doc Push the image to a container registry.
%%
%% ```
...
%% @doc Push the image to a container registry with authentication.
%%
%% ```
%% ok = ocibuild:push(Image, <<"ghcr.io">>, <<"myorg/myapp:v1.0.0">>,
%%                      #{token => os:getenv("GITHUB_TOKEN")}).
%% '''
-spec push(image(), Registry :: binary(), RepoTag :: binary(), auth()) -> ok | {error, term()}.
push(Image, Registry, RepoTag, Auth) ->
    {Repo, Tag} = parse_repo_tag(RepoTag),
    ocibuild_registry:push(Image, Registry, Repo, Tag, Auth).

%% @doc Save the image as an OCI layout tarball.
%%
...
%% @doc Export the image as an OCI layout directory.
%%
...
init_config(BaseConfig) ->
    ...
    #{
        ...
        <<"rootfs">> => maps:get(<<"rootfs">>, BaseConfig, #{
            <<"type">> => <<"layers">>,
            <<"diff_ids">> => []
        }),
